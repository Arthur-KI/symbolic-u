...
print("=== v5.6 AUTONOMOUS SALIENCE / CONCEPT MINING ===")
...
report={
    "version":"v5.6-autonomous-salience-concept-mining",
    "result":"PASS",
    "corpus":{
        "stories":len(CORPUS),
    ...
    "design":[
        ...
        "Salience requires recurrence across stories/domains, high structural pair precision, full variable closure, positive compression gain, connectedness, and non-degeneracy.",
...
Path("/mnt/data/symbolic_v56_autonomous_salience_report.json").write_text(
    json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8"
)
with Path("/mnt/data/symbolic_v56_autonomous_salience_checks.csv").open("w",newline="",encoding="utf-8") as f:
    w=csv.writer(f)
    w.writerow(["check","passed"])
    for k,v in checks.items():
        w.writerow([k,v])

print("\nSaved v5.6 report/checks.")
