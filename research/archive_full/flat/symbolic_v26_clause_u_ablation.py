...
# ============================================================
# Symbolic Mini-LM v2.6 — Clause-U ablation
#
# Distinction:
...
def extract_fill_chain(text:str):
    """
    Carry actor=BREI across coordinated elliptical NP list only inside asserted span.
    Stop propagation at hypothetical clause marker.
    """
    asserted=text
    m=re.search(r"\bals\s+wollt(?:e|s)?\b|\bals\s+ob\b",text,re.I)
    if m:
        asserted=text[:m.start()]
...
Path("/mnt/data/symbolic_v26_clause_u_report.json").write_text(
...
print("\nSaved /mnt/data/symbolic_v26_clause_u_report.json")
