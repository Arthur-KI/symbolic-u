...
def ref_variant(p:RefProbe, variant:str):
    ...
    c=morphology(p.surface,p.snippet)
    if variant in {"V2_ROLE","V3_ROLE_SCENE"}:
        c=add_role(c,p.snippet)
    allowed=p.scene if variant=="V3_ROLE_SCENE" else None
...
ADV_EVENT="Der Brei steht in der Küche, und Anna geht dann auf die Straße."
...
variants=["V0_RECENCY","V1_MORPH","V2_ROLE","V3_ROLE_SCENE"]
...
Path("/mnt/data/symbolic_v21_bottleneck_report.json").write_text(
    json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8"
)
print("\nSaved /mnt/data/symbolic_v21_bottleneck_report.json")
